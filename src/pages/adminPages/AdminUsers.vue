...
const userStats = computed(() => [
    { title: "Total Utilisateurs", value: users.value?.filter(u => u.role !== 'admin')?.length || 0, icon: "bi bi-people-fill" },
    { title: "Total Admins", value: users.value?.filter(u => u.role === 'admin')?.length || 0, icon: "bi bi-person-badge-fill" },
]);
...
const handleChangeRole = async (id, role) => {
    ...
    if (!error_changeRole.value)
        alert("Votre affectation de rôle a été effectué avec success")
    else alert("Votre affectation de rôle a échoué")
}
...
const deleteUser = async (id) => {
    if (confirm("Voulez-vous vraiment supprimer cet utilisateur ?")) {
        ...
        if (!error_removeUser.value)
            alert("Suppression de l'utilisateur a été effectué avec success")
        else alert("Suppression de l'utilisateur a échoué")
    }
}
